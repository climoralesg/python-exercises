...
takeoff = '2025*12*24@00|00|12 NP'

def get_time(time: str) -> datetime :
  zone_change_time = time.replace('NP','UTC')
  time_final = zone_change_time.rsplit(" ", 1)[0]
  time_object = datetime.strptime(time_final, '%Y*%m*%d@%H|%M|%S')
  return time_object
    

def time_until_take_off(from_time: str, take_off_time: str) -> int:
  deploy_time = get_time(from_time)
  actual_time = get_time(take_off_time)
  rest_time = deploy_time-actual_time
  return floor(rest_time.total_seconds())




if __name__ == "__main__":
  print(f"quedan {time_until_take_off('2025*12*25@00|00|00 NP', takeoff)} segundos")
